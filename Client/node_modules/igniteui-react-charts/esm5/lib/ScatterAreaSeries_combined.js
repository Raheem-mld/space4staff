/*
THIS INFRAGISTICS ULTIMATE SOFTWARE LICENSE  AGREEMENT ("AGREEMENT") LOCATED HERE:
https://www.infragistics.com/legal/license/igultimate-la
https://www.infragistics.com/legal/license/igultimate-eula
GOVERNS THE LICENSING, INSTALLATION AND USE OF INFRAGISTICS SOFTWARE. BY DOWNLOADING AND/OR INSTALLING AND USING INFRAGISTICS SOFTWARE:  you are indicating that you have read and understand this Agreement, and agree to be legally bound by it on behalf of the yourself and your company.
*/
import { __extends } from "tslib";
import { ScatterTriangulationSeries } from "./ScatterTriangulationSeries";
import { ColorScale } from "./ColorScale";
import { runOn, delegateRemove, delegateCombine, typeCast, Point_$type, markType, markDep } from "igniteui-react-core";
import { DependencyProperty } from "igniteui-react-core";
import { FastItemIntColumn } from "igniteui-react-core";
import { IFastItemsSource_$type } from "igniteui-react-core";
import { Rect } from "igniteui-react-core";
import { SeriesView } from "./SeriesView";
import { TriangleRasterizer } from "./TriangleRasterizer";
import { ScalerParams } from "./ScalerParams";
import { ObservableColorCollection } from "igniteui-react-core";
import { Color } from "igniteui-react-core";
import { ColorUtil } from "igniteui-react-core";
import { CustomPaletteColorScale } from "./CustomPaletteColorScale";
import { PropertyMetadata } from "igniteui-react-core";
import { truncate, intDivide } from "igniteui-react-core";
import { boxArray$1 } from "igniteui-react-core";
import { RenderingContext } from "igniteui-react-core";
import { CanvasViewRenderer } from "igniteui-react-core";
import { Rectangle } from "igniteui-react-core";
import { stringIsNullOrEmpty } from "igniteui-react-core";
/**
 * @hidden
 */
var ScatterAreaSeries = /** @class */ /*@__PURE__*/ (function (_super) {
    __extends(ScatterAreaSeries, _super);
    function ScatterAreaSeries() {
        var _this = _super.call(this) || this;
        _this.q1 = null;
        _this.q6 = null;
        _this.qz = null;
        _this.q0 = null;
        FastItemIntColumn.m();
        _this.ab = ScatterAreaSeries.$;
        return _this;
    }
    ScatterAreaSeries.prototype.get_c7 = function () {
        return true;
    };
    Object.defineProperty(ScatterAreaSeries.prototype, "q3", {
        get: function () {
            return this.q1;
        },
        set: function (a) {
            var b = this.q1 != a;
            if (b) {
                var c = this.q1;
                if (this.q1 != null) {
                    var d = this.q1;
                    d.propertyChanged = delegateRemove(d.propertyChanged, runOn(this, this.rg));
                }
                this.q1 = a;
                if (this.q1 != null) {
                    var e = this.q1;
                    e.propertyChanged = delegateCombine(e.propertyChanged, runOn(this, this.rg));
                }
                this.raisePropertyChanged("ColorScale", c, this.q1);
            }
        },
        enumerable: false,
        configurable: true
    });
    ScatterAreaSeries.prototype.rg = function (a, b) {
        this.ri();
        this.l0(false);
    };
    ScatterAreaSeries.prototype.hk = function (a, b) {
        var c = this.ja(b);
        if (c == ScatterAreaSeries.$$p[1]) {
            return this.hl(a, b, this.rb);
        }
        return _super.prototype.hk.call(this, a, b);
    };
    Object.defineProperty(ScatterAreaSeries.prototype, "q7", {
        get: function () {
            return this.q6;
        },
        set: function (a) {
            var b = this.q7 != a;
            if (b) {
                var c = this.q7;
                this.q6 = a;
                this.raisePropertyChanged("ColorColumn", c, this.q7);
            }
        },
        enumerable: false,
        configurable: true
    });
    ScatterAreaSeries.prototype.get_ex = function () {
        return true;
    };
    ScatterAreaSeries.prototype.set_ex = function (a) {
        _super.prototype.set_ex.call(this, a);
    };
    ScatterAreaSeries.prototype.lm = function (a, b, c, d) {
        _super.prototype.lm.call(this, a, b, c, d);
        switch (b) {
            case "FastItemsSource":
                var e = typeCast(IFastItemsSource_$type, c);
                if (e != null) {
                    e.deregisterColumn(this.q7);
                    this.q7 = null;
                }
                var f = typeCast(IFastItemsSource_$type, d);
                if (f != null) {
                    this.q7 = this.ce(this.rb);
                }
                this.l0(false);
                break;
            case ScatterAreaSeries.$$p[1]:
                if (this.ch != null) {
                    this.ch.deregisterColumn(this.q7);
                    this.q7 = this.ce(this.rb);
                }
                this.l0(false);
                this.k7();
                break;
            case "ColorScale":
                this.ri();
                this.l0(false);
                this.k7();
                break;
        }
    };
    ScatterAreaSeries.prototype.l1 = function (a) {
        _super.prototype.l1.call(this, a);
        this.rh(this.bx);
    };
    ScatterAreaSeries.prototype.lz = function (a, b, c, d, e) {
        _super.prototype.lz.call(this, a, b, c, d, e);
        var f = this.a4.a$h.item(d);
        f.a1(c);
        if (this.ct(f)) {
            return;
        }
        this.rh(f);
    };
    ScatterAreaSeries.prototype.rh = function (a) {
        if (this.ct(a)) {
            return;
        }
        var b;
        var c;
        var d = a.getViewInfo(c, b);
        c = d.p0;
        b = d.p1;
        var e = a;
        var f = null;
        var g = null;
        var h = this.pi;
        var i = this.pj;
        var j = this.getEffectiveViewport1(a);
        var k = new ScalerParams(1, b, c, h.cc);
        k.c = j;
        var l = new ScalerParams(1, b, c, i.cc);
        l.c = j;
        g = this.q5(a, k, l);
        if (g != null) {
            e.b8(g);
        }
        else {
            e.cb(f);
        }
    };
    ScatterAreaSeries.prototype.re = function (a) {
        var b;
        var c;
        var d = a.getViewInfo(b, c);
        b = d.p0;
        c = d.p1;
        var e = truncate(Math.round(b.width)) * truncate(Math.round(b.height));
        var f = a.b0;
        if (f == null || f.length != e) {
            f = new Array(e);
            for (var g = 0; g < e; g++) {
                f[g] = 0;
            }
        }
        else {
            for (var h = 0; h < e; h++) {
                f[h] = 0;
            }
        }
        a.b0 = f;
    };
    ScatterAreaSeries.prototype.hh = function (a) {
        var b = truncate(Math.round(this.pf.width));
        var c = truncate(Math.round(this.pf.height));
        var d = this.bx.b0;
        if (d == null || d.length != (b * c) || this.ps == null || this.ps.count < 1 || this.pt == null || this.pt.count < 1 || this.pu == null || this.pu.count < 1) {
            return null;
        }
        var e = this.ca.pw;
        var f = (a.x - e.left) / e.width;
        var g = (a.y - e.top) / e.height;
        var h = truncate(Math.round(b * f));
        var i = truncate(Math.round(c * g));
        var j = (b * i) + h;
        if (j < 0 || j > d.length - 1) {
            return null;
        }
        var k = d[j] - 1;
        if (k < 0 || k > this.ps.count) {
            return null;
        }
        var l = this.qz.length;
        var m = this.ps.item(k);
        var n = this.pt.item(k);
        var o = this.pu.item(k);
        if ((m >= l) || (n >= l) || (o >= l)) {
            return null;
        }
        var p = (this.qz[m] - this.pf.left) / this.pf.width;
        var q = (this.q0[m] - this.pf.top) / this.pf.height;
        var r = (this.qz[n] - this.pf.left) / this.pf.width;
        var s = (this.q0[n] - this.pf.top) / this.pf.height;
        var t = (this.qz[o] - this.pf.left) / this.pf.width;
        var u = (this.q0[o] - this.pf.top) / this.pf.height;
        var v = Math.pow(p - f, 2) + Math.pow(q - g, 2);
        var w = Math.pow(r - f, 2) + Math.pow(s - g, 2);
        var x = Math.pow(t - f, 2) + Math.pow(u - g, 2);
        if (v < w && v < x) {
            return this.ch.item(m);
        }
        if (w < v && w < x) {
            return this.ch.item(n);
        }
        if (x < v && x < w) {
            return this.ch.item(o);
        }
        return this.ch.item(m);
    };
    ScatterAreaSeries.prototype.q5 = function (a, b, c) {
        var d;
        var e;
        var f = a.getViewInfo(e, d);
        e = f.p0;
        d = f.p1;
        e = new Rect(0, 0, 0, e.width, e.height);
        b = new ScalerParams(0, b.e, e, b.a, b.c);
        c = new ScalerParams(0, c.e, e, c.a, c.c);
        c.b = this.f2();
        var g = a;
        var h = this.pi;
        var i = this.pj;
        var j = this.ps.count;
        var k;
        var xArr_ = this.pv.asArray();
        k = (xArr_.slice(0));
        var l;
        var yArr_ = this.pw.asArray();
        l = (yArr_.slice(0));
        var m = this.q7.asArray();
        var n = this.ps.asArray();
        var o = this.pt.asArray();
        var p = this.pu.asArray();
        this.qz = k;
        this.q0 = l;
        h.g5(boxArray$1(k), 0, k.length, b);
        i.g5(boxArray$1(l), 0, l.length, c);
        var q = this.q2;
        var r = g.b2(b.d, q, this.q7);
        var s = e.top;
        var t = e.left;
        var u = e.right;
        var v = e.bottom;
        var w;
        var x;
        var y;
        var z;
        this.re(g);
        var aa = g.b0;
        var ab = k.length;
        for (var ac = 0; ac < j; ac++) {
            var ad = n[ac];
            var ae = o[ac];
            var af = p[ac];
            if ((ad >= ab) || (ae >= ab) || (af >= ab) || (ad < 0) || (ae < 0) || (af < 0)) {
                continue;
            }
            var ag = k[ad];
            var ah = l[ad];
            var ai = { $type: Point_$type, x: ag, y: ah };
            var aj = k[ae];
            var ak = l[ae];
            var al = { $type: Point_$type, x: aj, y: ak };
            var am = k[af];
            var an = l[af];
            var ao = { $type: Point_$type, x: am, y: an };
            w = al.y < ao.y ? al.y : ao.y;
            w = ai.y < w ? ai.y : w;
            x = al.y > ao.y ? al.y : ao.y;
            x = ai.y > x ? ai.y : x;
            y = al.x < ao.x ? al.x : ao.x;
            y = ai.x < y ? ai.x : y;
            z = al.x > ao.x ? al.x : ao.x;
            z = ai.x > z ? ai.x : z;
            if (w < v && x > s && y < u && z > t) {
                var ap = m[ad];
                var aq = m[ae];
                var ar = m[af];
                r.p(ac, aa, ai, al, ao, ap, aq, ar);
            }
        }
        return r;
    };
    ScatterAreaSeries.prototype.q8 = function () {
        var a = new Color();
        var b = this.of;
        if (b == null) {
            a = ColorUtil.r("#000000");
        }
        else {
            a = ColorUtil.q(b);
        }
        var c = new ObservableColorCollection();
        c.add(ColorUtil.v(a, -0.5));
        c.add(a);
        return c;
    };
    ScatterAreaSeries.prototype.ri = function () {
        var _this = this;
        var a = this.q3;
        if (a == null) {
            a = ((function () {
                var $ret = new CustomPaletteColorScale();
                $ret.palette = _this.q8();
                $ret.interpolationMode = 2;
                return $ret;
            })());
        }
        else if (typeCast(CustomPaletteColorScale.$, a) !== null) {
            var b = a;
            if (b.palette == null || b.palette.count == 0) {
                b.palette = this.q8();
            }
            a = b;
        }
        var c = new ObservableColorCollection();
        if (typeCast(CustomPaletteColorScale.$, a) !== null) {
            c = a.palette;
        }
        else {
            c = this.q8();
        }
        this.q2 = a;
    };
    ScatterAreaSeries.prototype.md = function () {
        _super.prototype.md.call(this);
        this.ri();
    };
    ScatterAreaSeries.prototype.kh = function (a, b) {
        _super.prototype.kh.call(this, a, b);
        var c = b;
        c.b7();
    };
    ScatterAreaSeries.prototype.bu = function () {
        return new ScatterAreaSeriesView(this);
    };
    ScatterAreaSeries.prototype.lh = function (a) {
        _super.prototype.lh.call(this, a);
        this.q4 = typeCast(ScatterAreaSeriesView.$, a);
    };
    ScatterAreaSeries.prototype.rf = function (a) {
        if (this.q4 != null) {
            this.q4.b6(a);
        }
    };
    ScatterAreaSeries.$t = markType(ScatterAreaSeries, 'ScatterAreaSeries', ScatterTriangulationSeries.$);
    ScatterAreaSeries.$$p = markDep(DependencyProperty, PropertyMetadata, ScatterAreaSeries, 'raisePropertyChanged', ['ActualColorScale:q2:rj', [ColorScale.$, null], 'ColorMemberPath:rb:rk', [2, null]]);
    return ScatterAreaSeries;
}(ScatterTriangulationSeries));
export { ScatterAreaSeries };
/**
 * @hidden
 */
var ScatterAreaSeriesView = /** @class */ /*@__PURE__*/ (function (_super) {
    __extends(ScatterAreaSeriesView, _super);
    function ScatterAreaSeriesView(a) {
        var _this = _super.call(this, a) || this;
        _this.b1 = null;
        _this.bx = null;
        _this.b5 = null;
        _this.b4 = null;
        _this.b3 = null;
        _this.b1 = a;
        return _this;
    }
    Object.defineProperty(ScatterAreaSeriesView.prototype, "b0", {
        get: function () {
            return this.bx;
        },
        set: function (a) {
            this.bx = a;
        },
        enumerable: false,
        configurable: true
    });
    ScatterAreaSeriesView.prototype.ca = function (a, b) {
    };
    ScatterAreaSeriesView.prototype.b6 = function (a) {
    };
    ScatterAreaSeriesView.prototype.by = function () {
        return this.bz(true);
    };
    ScatterAreaSeriesView.prototype.bz = function (a) {
        this.b9();
        var b = truncate(Math.round(this.bu.left));
        var c = truncate(Math.round(this.bu.left));
        var d = truncate(Math.round(this.bu.width));
        var e = truncate(Math.round(this.bu.height));
        var f = d.toString();
        var g = e.toString();
        if (this.b4.getAttribute("width") != f) {
            this.b4.setAttribute("width", d.toString());
        }
        if (this.b4.getAttribute("height") != g) {
            this.b4.setAttribute("height", e.toString());
        }
        var h = this.b3.g();
        if (a) {
            h.clearRect(0, 0, d, e);
        }
        this.b5 = h.getImageData(0, 0, d, e);
        var imageDataArray_ = this.b5.data;
        return imageDataArray_;
    };
    ScatterAreaSeriesView.prototype.b9 = function () {
        if (this.b4 == null) {
            this.b4 = this.e.gw.createElement("canvas");
            var a = this.e.gw.get2DCanvasContext(this.b4);
            this.b3 = new RenderingContext(new CanvasViewRenderer(), a);
        }
    };
    ScatterAreaSeriesView.prototype.b2 = function (a, b, c) {
        var d = truncate(Math.round(this.bu.left));
        var e = truncate(Math.round(this.bu.left));
        var f = truncate(Math.round(this.bu.width));
        var g = truncate(Math.round(this.bu.height));
        return new TriangleRasterizer(this.by(), b, c.minimum, c.maximum, c, f, g);
    };
    ScatterAreaSeriesView.prototype.b8 = function (a) {
        this.cb(null);
    };
    ScatterAreaSeriesView.prototype.b7 = function () {
        if (this.b3 != null) {
            this.b3.k(this.bu.left, this.bu.top, this.bu.width, this.bu.height);
        }
        this.aq();
    };
    ScatterAreaSeriesView.prototype.a6 = function (a, b) {
        _super.prototype.a6.call(this, a, b);
        if (this.b4 == null) {
            return;
        }
        if (b) {
            var c = new Rectangle();
            c.n = this.bu.left;
            c.o = this.bu.top;
            c.width = this.bu.width;
            c.height = this.bu.height;
            c._fill = this.bo();
            a.v(c);
        }
        else {
            var d = truncate(Math.round(this.bu.left));
            var e = truncate(Math.round(this.bu.top));
            var f = truncate(Math.round(this.bu.width));
            var g = truncate(Math.round(this.bu.height));
            a.m(this.b4.getNativeElement(), 1, d, e, f, g);
        }
    };
    ScatterAreaSeriesView.prototype.cb = function (a) {
        var b = truncate(Math.round(this.bu.left));
        var c = truncate(Math.round(this.bu.left));
        var d = truncate(Math.round(this.bu.width));
        var e = truncate(Math.round(this.bu.height));
        this.b9();
        this.b3.g().putImageData(this.b5, 0, 0);
        this.aq();
    };
    ScatterAreaSeriesView.prototype.al = function (a) {
        _super.prototype.al.call(this, a);
        var b = truncate(Math.round(this.bu.width));
        var c = truncate(Math.round(this.bu.height));
        var d = this.bz(false);
        if (d == null || d.length == 0) {
            this.e.a6.exportSeriesPixelsData(a, null, b, c);
            return;
        }
        var e = new Array(intDivide(d.length, 4));
        var f;
        for (var g = 0; g < intDivide(d.length, 4); g++) {
            f = g * 4;
            e[g] = d[f + 3] << 24 | d[f] << 16 | d[f + 1] << 8 | d[f + 2];
        }
        this.e.a6.exportSeriesPixelsData(a, e, b, c);
    };
    ScatterAreaSeriesView.prototype.getDefaultTooltipTemplate = function () {
        var a = "<div class='ui-chart-default-tooltip-content'><span";
        var b = this.e.a4.a$r(this.e);
        if (!stringIsNullOrEmpty(b)) {
            a += " style='color:" + b + "'";
        }
        a += ">" + this.b1.hp + "</span><br/><span>" + "(${item." + this.b1.qg + "}, ${item." + this.b1.qk + "})</span>";
        if (!stringIsNullOrEmpty(this.b1.rb)) {
            a += "<span>, Value: ${item." + this.b1.rb + "}</span>";
        }
        a += "</div>";
        return a;
    };
    ScatterAreaSeriesView.$t = markType(ScatterAreaSeriesView, 'ScatterAreaSeriesView', SeriesView.$);
    return ScatterAreaSeriesView;
}(SeriesView));
export { ScatterAreaSeriesView };
//# sourceMappingURL=ScatterAreaSeries_combined.js.map
